...
void MainWindow::setRenderMesh(const UIMesh& mesh) {
    ...
    mMeshExplorer->setPointsValueLabel(mesh.pointsReal().size());
    mMeshExplorer->setConstraintsValueLabel(mesh.constraints().size());
    mMeshExplorer->setTrianglesValueLabel(mesh.triangles().size());
}
